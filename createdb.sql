...
create table if not exists land( id int, x int, y int, width int, height int);
create unique index idx_land_id on land( id);

create table if not exists city_resource( id int, resource_type_id int, resource_amount int);
create unique index idx_city_resource_id on city_resource(id);

create table if not exists player(id int, name varchar(64), capital_city_id int);
create unique index idx_player_id on player(id);

create table if not exists city( id int, player_id int, name varchar(64), food int, gold int, tax_rate float, captil_flag int, land_id int);
